...
const TopicManagement = () => {
  ...
  const fetchPendingTopics = async () => {
    ...
    if (error) {
      ...
      // Transform database topics to match our Topic interface
      const transformedTopics: Topic[] = (data || []).map((dbTopic: DatabaseTopic) => ({
        id: dbTopic.id,
        title: dbTopic.title,
        theme: dbTopic.theme || 'General',
        description: dbTopic.description,
        status: ['pending', 'approved', 'rejected'].includes(dbTopic.status)
          ? dbTopic.status as 'pending' | 'approved' | 'rejected'
          : 'pending',
        created_at: dbTopic.created_at
      }));
      setPendingTopics(transformedTopics);
    }
    setLoading(false);
  };
