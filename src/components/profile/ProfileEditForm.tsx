...
import { BookOpen, Globe, Trophy, Film, Scale, Cpu, Heart } from 'lucide-react';
...
const ProfileEditForm = ({ onSuccess, initialData }: ProfileEditFormProps) => {
  const { user, updateProfile } = useCustomAuth();
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);

  const [formData, setFormData] = useState({
    fullName: initialData?.full_name || user?.full_name || '',
    age: initialData?.age || user?.age || '',
    gender: initialData?.gender || user?.gender || '',
    country: initialData?.country || user?.country || '',
    school: initialData?.school || user?.school || '',
    interests: initialData?.interests || user?.interests || [] as string[]
  });
  ...
  const handleInterestToggle = (interestId: string) => {
    setFormData(prev => {
      ...
      if (currentInterests.includes(interestId)) {
        return { ...prev, interests: currentInterests.filter((i: string) => i !== interestId) };
      } else {
